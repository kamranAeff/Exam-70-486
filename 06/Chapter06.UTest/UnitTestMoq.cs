...
namespace Chapter06.UTest
{
    ...
    public class UnitTestMoq
    {
        ...
        public void Pass_Through_Variable_Discounts()
        {
            // arrange
            Mock<IDiscountHelper> mock = new Mock<IDiscountHelper>();
            Debug.Print("It.IsIn: {0}\n", It.Is<decimal>(v=>v==0));
            mock.Setup(m => m.ApplyDiscount(It.IsAny<decimal>()))
            .Returns<decimal>(total => {
                Debug.WriteLine("Mock - IDiscountHelper.Setup-1");
                return total;
            });
            ...
            .Returns<decimal>(total => {
                Debug.WriteLine("Mock - IDiscountHelper.Setup-3");
                return (total * 0.9M); });
            mock.Setup(m => m.ApplyDiscount(It.IsInRange<decimal>(10, 100,
            Range.Inclusive))).Returns<decimal>(total => {
                Debug.WriteLine("Mock - IDiscountHelper.Setup-4");
                return total - 5;
            });
            var target = new LinqValueCalculator(mock.Object);
